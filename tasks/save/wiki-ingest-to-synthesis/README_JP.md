...
*OpenClaw WebUI —— エンドユーザーの依頼：「LLM Wiki が RAG より長期の知識管理に向いている理由について 300 字の WeChat 記事を書いて」。ユーザーは「wiki」にも触れず、ソースリンクも貼っていない。それでも下書きには `Karpathy 的 LLM Wiki` ページからの「概念を抽出し、矛盾にフラグを立て、クロスリファレンスを築く」といった表現がそのまま現れ、「RAG は探すのが得意、Wiki は育てるのが得意」というキャッチコピーは `RAG 与 Wiki 的对比` から合成されている。注入は PowerMem プラグインの `before_agent_start` フックが自動で行う。*
